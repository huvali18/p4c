...
#include "backends/p4tools/modules/smith/common/expression_int.h"
...
IR::Expression *expression_bit::createSaturationOperand(const IR::Type_Bits *tb) {
    IR::Expression *expr = construct(tb);
    ...
    switch (randInd(percent)) {
        ...
        case 7: {
            ...
            if (P4Scope::constraints.const_lshift_count) {
                right =
                    BaseType::gen_bit_literal(new IR::Type_Bits(P4Scope::req.shift_width, false));
            } else {
...
IR::Expression *expression_bit::construct_ternary_expr(const IR::Type_Bits *tb) {
    IR::Expression *expr = nullptr;
